...
class TestConductor:
    ...
    def go(self, options):
        ...
        try:
            ...
            stage.do_build(
                32, python.Results('32-bit build results'), options.get_externals
            )
            options.just_build or stage.run_test(python.Results('32-bit test results'))
            if not options.skip_64_bit:
                stage.do_build(
                    64, python.Results('64-bit build results'), options.get_externals
                )
