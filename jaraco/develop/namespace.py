...
def load_template(name, transform=lambda x: x, context=None):
...
def create_namespace_package_cmd():
    ...
    parser.add_argument(
        '-s', '--indent-with-spaces', default=False, action='store_true',
    )
    parser.add_argument('target', help="path to new project", type=Path)
...
def create_namespace_sandbox(root='.'):
    ...
    (testdir / 'test_basic.py').open('w').write(
        DALS(
            """
