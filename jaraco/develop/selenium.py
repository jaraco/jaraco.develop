...
def find_firefox_win32():
    ...
    key = _winreg.OpenKey(
        _winreg.HKEY_LOCAL_MACHINE, r'Software\Wow6432Node\Mozilla\Mozilla Firefox'
    )
...
def get_options():
    ...
    parser.add_option(
        '-j', '--java-vm', default=r'c:\program files\java\jdk\bin\java.exe'
    )
    parser.add_option(
        '-s',
        '--selenium-jar',
        default=r'c:\program files\java\selenium-server\selenium-server.jar',
    )
...
def get_saucelabs_params(os='Windows 2003', browser=browsers.ie7, job_name=None):
    ...
    params = dict(username=username, os=os, browser=browser.name,)
