...
def create_repository(name, auth, private=False):
    resp = requests.post(
        make_url('repositories/'),
        data=dict(name=name, is_private=private, scm='hg', language='python'),
        auth=auth,
...
def add_version(project, version, auth):
    ...
    resp = requests.post(
        url, params=dict(name=version), auth=auth, headers=dict(Accept='text/json'),
    )
    handle_error(resp)
    return resp.json()
...
def create_repository_cmd():
    ...
    res = create_repository(args.repo_name, args.auth, private=args.private)
    pprint.pprint(res)
...
def update_wiki(project, title, path, content):
    ...
    resp = requests.put(
        url, data=data, auth=get_mercurial_creds(), headers=dict(Accept='text/json')
    )
    handle_error(resp)
